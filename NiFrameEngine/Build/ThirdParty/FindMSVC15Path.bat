...
set scriptpath=%~dp0
for /f "usebackq tokens=1* delims=" %%i in (`%scriptpath%vswhere.exe -latest -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath`) do (
	set VS2017DIR=%%i
)
...
set MSVC15DIRX86=%VS2017DIR%\VC\Tools\MSVC\%VCTOOLSVERSION%\bin\HostX64\x86
